 ...
 */
class SliderField extends \CleverSoft\CleverBuilder\Helper\RenderFields\BaseFieldAbs {
    /*
     *
     */
    protected function get_input_classes() {
        return array( 'widefat', 'cleversoft-slider-input' ,'form-control','add-click-ui-slider');
    }
    ...
    protected function render_field( $value, $instance ) {
        ...
        ?>
        <div data-mage-init='{"CleverSoft_CleverBuilder/js/slider-ui":{
                    "min":<?php echo (int)$this->field_options['min'] ?>,
                    "max":<?php echo (int)$this->field_options['max'] ?>,
                    "elementId":"<?php echo $id ?>",
                    "value":<?php echo $value ? $value : (isset($this->field_options['value']) ? $this->field_options['value'] : $this->field_options['min']) ?>
                    }}'></div>
        <input <?php if(isset($this->field_options['depends'])) : ?> 
            ...
            id="<?php echo ( $id ) ?>"
            value="<?php echo ( $value ) ?>" <?php $this->render_data_attributes( $this->get_input_data_attributes() ) ?>
                          ...
                          case 'style':
                            if($data_code){
                              <?php if ((isset($this->field_options['unit']) && $this->field_options['unit']) || (isset($this->field_options['prefix']) && $this->field_options['prefix'])):?>
                                  <?php if ((isset($this->field_options['unit']) && $this->field_options['unit']) && (isset($this->field_options['prefix']) && $this->field_options['prefix'])):?>
                                    classEl.css('<?=isset($this->field_options['code']) ? $this->field_options['code'] : ''?>','<?=$this->field_options['prefix']?>'+$content+'<?=$this->field_options['unit']?>');
                                  <?php else:?>
                                    <?php if (isset($this->field_options['prefix']) && $this->field_options['prefix']):?>
                                      classEl.css('<?=isset($this->field_options['code']) ? $this->field_options['code'] : ''?>','<?=$this->field_options['prefix']?>'+$content);
                                    <?php endif;?>
