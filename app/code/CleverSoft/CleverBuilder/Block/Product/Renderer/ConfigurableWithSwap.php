...
use CleverSoft\CleverBuilder\Helper\Product\Data as BaseHelper;
use CleverSoft\CleverBuilder\Helper\Product\Image as BaseImageHelper;
 ...
 * @since 100.0.2
 */
class ConfigurableWithSwap extends \Magento\ConfigurableProduct\Block\Product\View\Type\Configurable implements
    \Magento\Framework\DataObject\IdentityInterface
{
    ...
    public function __construct(
        ...
        $this->_baseImageHelper = $baseImageHelper;
    ...
    protected function getVariationMedia($attributeCode, $optionId)
    {
        ...
        if ($variationProduct) {
            $variationMediaArray = [
                ...
                'swap' => $this->getSwapProductImage($variationProduct, Swatch::SWATCH_IMAGE_NAME),
            ];
        }
     ...
     * @return string
     */
    protected function getSwapProductImage(Product $childProduct, $imageType)
    {
        if ($this->isProductHasImage($childProduct, Swatch::SWATCH_IMAGE_NAME)) {
            $swatchImageId = $imageType;
        } elseif ($this->isProductHasImage($childProduct, 'image')) {
        ...
        if (isset($swatchImageId)) {
            $imageWidth = $this->_baseHelper->getCfg("category/image_width");
            $imageHeight = $this->_baseHelper->getCfg("category/image_height");

            $keepAspectRatio = $this->_baseHelper->getCfg('category/aspect_ratio');

            if ($keepAspectRatio)
            {
                $imageHeight = 0; //Height will be calculated automatically (based on width) to keep the aspect ratio
            }

            return $this->_baseImageHelper->getImg($childProduct, $imageWidth, $imageHeight,'category_page_grid');
        }
