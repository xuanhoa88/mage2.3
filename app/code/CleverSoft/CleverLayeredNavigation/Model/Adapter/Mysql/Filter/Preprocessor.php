 ...
 */
namespace CleverSoft\CleverLayeredNavigation\Model\Adapter\Mysql\Filter;
...
use CleverSoft\CleverLayeredNavigation\Model\Adapter\Mysql\Filter\AliasResolver as AliasResolver;

/**
 * @SuppressWarnings(PHPMD.CouplingBetweenObjects)
 */
class Preprocessor extends \Magento\CatalogSearch\Model\Adapter\Mysql\Filter\Preprocessor
{
    /**
    ...
    public function __construct(
        ...
        if (null === $aliasResolver) {
            $aliasResolver = $objectManager->get(AliasResolver::class);
        }
    ...
    private function processQueryWithField(FilterInterface $filter, $isNegation, $query)
    {
        ...
        if ($filter->getField() === 'price') {
            ...
            if(is_array($filter->getValue())) {
                $filter_cat = implode(',',$filter->getValue());
                if (version_compare($this->_version->getVersion(),  '2.2.0', '>=')) {
                    return 'category_ids_index.category_id IN (' . $filter_cat . ')';
                } else {
                    return 'category_products_index.category_id IN (' . $filter_cat . ')';
                }
            }
            if (version_compare($this->_version->getVersion(),  '2.2.0', '>=')) {
                return 'category_ids_index.category_id = ' . (int) $filter->getValue();
            } else {
                return 'category_products_index.category_id = ' . (int) $filter->getValue();
            }
            ...
            if (version_compare($this->_version->getVersion(),  '2.2.0', '>=')) {
                if ($filter->getField() === \Magento\CatalogSearch\Model\Search\FilterMapper\VisibilityFilter::VISIBILITY_FILTER_FIELD) {
                    return '';
                }
            }
