define([
    ...
    (function( $, wpcustomize ) {
        ...
        var is_rtl = Footer_Layout_Builder.is_rtl;

        var FooterBuilder = function( options, id ){
            ...
            var Builder = {
                ...
                gridster: function( $wrapper, ui, event ){
                    ...
                    var mergeItem = function( node , newX, el){
                        var row_id = $(".footer-full-overlay" + " " + "#"+ el.parent().attr('id') +" .grid-stack-item");
                        var max_y = 1;
                        ...
                        if ( _.isUndefined(window.footer_max_y)) {
                            window.footer_max_y = max_y;
                        }
                        ...
                        if ( _.isUndefined(window.footer_min_height)) {
                            window.footer_min_height = parseInt($(".footer-full-overlay" + " " + "#"+ el.parent().attr('id')).css('min-height'))/window.footer_max_y;
                        }
                        $(".footer-full-overlay" + " " + "#"+ el.parent().attr('id')).css('min-height',max_y*window.footer_min_height+'px');
                    }
                ...
                updateAllGrids: function(){
                    ...
                    if (!$(".footer-full-overlay .cleversoft--panel-desktop .cleversoft--cp-rows .grid-stack-item").length) {
                        $('#footerbuilder-desktop-data').val('{}');
                    }
                    if (!$(".footer-full-overlay .cleversoft--panel-mobile .cleversoft--cp-rows .grid-stack-item").length) {
                        $('#footerbuilder-mobile-data').val('{}');
                    }
                },
                ...
                updateGridData: function($row) {
                    ...
                    if ($row.closest('.footer-full-overlay .cleversoft--device-panel').attr('data-device') == 'desktop') {
                        $(".cleversoft--cb-row", $row.closest('.footer-full-overlay .cleversoft--device-panel')).each(function(e) {
                            var i = $(this);
                        ...
                        $('#footerbuilder-desktop-data').val(JSON.stringify(data_desktop));
                    } else if($row.closest('.footer-full-overlay .cleversoft--device-panel').attr('data-device') == 'mobile') {
                        $(".cleversoft--cb-row", $row.closest('.footer-full-overlay .cleversoft--device-panel')).each(function(e) {
                            var i = $(this);
                            $(".grid-stack-item", i).each(function(e) {
                                var pos = $(this).parent().attr('data-id');
                        ...
                        $('#footerbuilder-mobile-data').val(JSON.stringify(data_mobile));
                    }
                ...
                addPanel: function( device ){
                    ...
                    var template_id =  'footer-cleversoft--cb-panel';
                ...
                addItem: function( node ){
                    var that = this;
                    var template = that.getTemplate();
                    var template_id =  'footer-cleversoft--cb-item';
                    if (  $( '#'+template_id ).length == 0 ) {
                        return ;
                    }
                ...
                clearData: function() {
                    var that = this;
                    $("#footer_clear_section").click(function() {
                        var device = $(".footer-full-overlay .cleversoft--tab-active").attr("data-device");
                        var items = $(".footer-full-overlay .cleversoft--panel-"+device+" .cleversoft--cp-rows .grid-stack-item, .cleversoft--panel-"+device+" .cleversoft--cp-sidebar .grid-stack-item");
                        var panel = $(".footer-full-overlay .cleversoft--device-panel[data-device='"+device+"']");
                        items.attr( 'data-gs-width', 1 );
                        items.attr( 'data-gs-x', 0 );
                        items.attr( 'data-gs-y', '' );
                        items.removeAttr( 'style' );
                        $(".footer-full-overlay .cleversoft--panel-"+device+" .cleversoft--cb-items").removeAttr( 'style' );
                        $( '.cleversoft-available-items', panel ).append( items );
                        that.updateAllGrids();
                        that.save();

                        $(".footerbuilder-modal").css("display","none");
                    });
                },
                ...
                focus: function(){
                    this.container.on("click", ".cleversoft--cb-item-setting, .cleversoft--cb-item-name, .item-tooltip", function(e) {
                        e.preventDefault();
                        var p = "footerbuilder-popup-"+$(this).data("id");
                        $(".footerbuilder-modal").css("display","none");
                        var modal = document.getElementById(p);

                        if (modal) {
                            $(".footerbuilder-modal-body .modal-inner").css("display","none");
                            var modal_content = document.getElementById("footer_" + $(this).closest(".cleversoft--device-panel").attr("data-device")+"_"+$(this).data("id"));
                            if (modal_content) {
                    ...
                    }),$(".footerbuilder-modal-close").click(function() {
                        $(".footerbuilder-modal").css("display","none");
                    })
                ...
                remove: function(){
                    ...
                    $document.on( 'click', '.cleversoft--device-panel .cleversoft--cb-item-remove', function ( e ) {
                        ...
                        if ( _.isUndefined(window.footer_min_height)) {
                            window.footer_min_height = parseInt($("#"+ item.parent().attr('id')).css('min-height'))/max_y;
                        }
                        if (max_y > 1) {
                            $("#"+ item.parent().attr('id')).css('min-height',(max_y-1)*window.footer_min_height+'px');
                        }
                ...
                init: function( controlId, items, devices ){
                    ...
                    var template = that.getTemplate();
                    var template_id =  'footer-cleversoft--builder-panel';
                    var html = template( options , template_id );
                    that.container = $( html );
                    $( 'body .footer-full-overlay' ).append( that.container );
                    that.controlId = controlId;
                    that.items = items;
                    that.devices = devices;

                    if ( options.section ) {
                        wpcustomize.section( options.section ).container.addClass( 'cleversoft--hide' );
                    }
        ...
        $(document).ready(function(e,b) {
            _.each( Footer_Layout_Builder.builders, function( opts, id ){
                new FooterBuilder( opts, id );
            } );
