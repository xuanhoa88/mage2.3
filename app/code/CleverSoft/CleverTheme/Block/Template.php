...
class Template extends \Magento\Framework\View\Element\Template
{
    ...
    public function __construct(
        ...
        \CleverSoft\CleverTheme\Helper\HeaderData $headerData,
        array $data = []
    ) {
...
HTML;
    ...
    public function renderCollection($template='header.phtml'){
        ...
        $this->setData(array(
            'full_header_width'            => $this->getConfig('cleversofttheme/header/full_header_width'),
            'compare'            => $this->getConfig('cleversofttheme/header/compare'),
            'wishlist'            => $this->getConfig('cleversofttheme/header/wishlist')
        ));
    ...
    public function getSettingData() {
        return $this->_headerData->getSettingData();
    }

    public function getHeaderDesktop() {
        $settings = $this->getSettingData();
        $storeId = $this->_storeManager->getStore()->getId();
        if ($this->_collectionFactory->create()->addFieldToFilter('store_id',$storeId)->getFirstItem()->getId() && isset($settings->{'onoffswitch-headerbuilder'}) && $settings->{'onoffswitch-headerbuilder'}) {
            return $this->_collectionFactory->create()->addFieldToFilter('store_id',$storeId)->getFirstItem()->getHeaderDesktopData();
        }
        return $this->_collectionFactory->create()->addFieldToFilter('store_id',0)->getFirstItem()->getHeaderDesktopData();
    }
    ...
    public function renderRow($items, $id = '', $device = 'desktop') {
        ...
        while ( $index < $n ) {
            ...
            $item_id    = $item->id;
            if ($device == "desktop") {
                $merge_key  = "_customize_header_".$item_id."_merge_item";
            } else {
                $merge_key  = "_customize_header_".$item_id."_merge_item_mobile";
            }

            $merge      = isset($this->_headerData->getSettingData()->{$merge_key}) && $this->_headerData->getSettingData()->{$merge_key} ? $this->_headerData->getSettingData()->{$merge_key} : "";
            ...
            if ( $next_item ) {
                if ($device == "desktop") {
                    $merge_key_next = "_customize_header_".$next_item->id."_merge_item";
                } else {
                    $merge_key_next = "_customize_header_".$next_item->id."_merge_item_mobile";
                }
                $merge_next     = isset($this->_headerData->getSettingData()->{$merge_key_next}) && $this->_headerData->getSettingData()->{$merge_key_next} ? $this->_headerData->getSettingData()->{$merge_key_next} : "";
            }
